...
// maximum number of words in any
#define WORDS 172806
...
/** 
 * Prints the grid in its current state. DONE BY DEV
 */
void draw(void)
{
 ...
 * as found (so that user can't score with it again) and returns true. DONE BY DEV
 */
bool lookup(string s)
{
    ...
    while (max > min)
    {
        ...
        {
            if (dictionary.words[midpoint].found == 1) //if the word has already been found return false
                return false;
            else
            {
                dictionary.words[midpoint].found = 1; //mark the word as found and return true
                return true;
            }
        }
        else if (strcmp (s, dictionary.words[midpoint].letters) > 0)
            min = midpoint + 1;
        else
            max = midpoint - 1;
    }
 ...
 * grid[0][0] rotates to grid[0][DIMENSION - 1]. DONE BY DEV
 */
void scramble(void)
{
    ...
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int j = 0; j < DIMENSION; j++)
        {
            newgrid[j][(DIMENSION - 1) - i] = grid[i][j];
        }
    }
